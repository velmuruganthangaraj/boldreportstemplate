...
$(document).ready(function () {
    ...
    $('#tenant-type').change(function () {
        ...
        if (dropdownValue === "BoldReportsOnPremise") {
            item = "reports";
            $("#header-description").html("Bold Reports multi-tenancy can host multiple sites on a single instance by isolating the database and the resources for each site.");
            $(".site-default-text").html("").html("i.e. " + boldReportsUrl);
        }
        else if (dropdownValue === "BoldBIOnPremise") {
            item = "dashboards";
            $("#header-description").html("Bold BI multi-tenancy can host multiple sites on a single instance by isolating the database and the resources for each site.");
    ...
    $("#tenant-identifier").validate({
        ...
        messages: {
            tenantidentifier: {
                isRequired: "Please enter site identifier.",
                isValidIdentifier: "Please avoid special characters."
            }
        }
    });
    ...
    $("#tenant-registration-form").validate({
        ...
        messages: {
            tenantname: {
                isRequired: "Please enter site name.",
                isValidName: "Please avoid special characters."
            },
            tenantidentifier: {
                required: "Please enter site identifier.",
                isValidIdentifier: "Please avoid special characters."
            }
        }
    });
        ...
        if ($(".tenant-database-form").hasClass("hide") && $(this).hasClass("database") && $("#tenant-registration-form").find(".has-error").length == 0) {
            ...
            if ($(".tenant-database-form #db-content-holder").valid()) {
                ...
                if (!$(".database-error").is(":visible")) {
                    ...
                    preserveFormData();
                    $("#header-title").html("Configure Data Store");
                    $("#header-description").html("Pull your data from " + dataConnectorsCount + "+ data connectors and save it offline to access your dashboard quickly.");
                    moveStepper("front", 3);
            ...
            if (!$(".database-error").is(":visible")) {
                ...
                if ($(".tenant-intermediate-database-form #db-content-holder").valid() || ($("#skip-intermediate-db").is(":visible") && $("#skip-intermediate-db").is(":checked"))) {
                    ...
                    $("#header-title").html("Select Site Administrator");
                    $("#header-description").text("Administrators have complete control over the site.");
    ...
    $(document).on("click", "#details-back", function () {
        if ($("#details-next").hasClass("intermediate-db") || $("#details-next").hasClass("update")) {
            ...
            $("#header-title").html("Site Creation");
            if (item === "dashboards") {
                $("#header-description").html("Bold BI multi-tenancy can host multiple sites on a single instance by isolating the database and the resources for each site.");
            }
            else {
            ...
            $("#details-next").attr("value", "Next").removeClass("next-alignment");
        }
            ...
            $("#no-need-datastore").css("display", "none");
            moveStepper("back", 2);
            $("#header-title").html("Select Database");
            $("#header-description").text("The place where you will create and share your dashboards.").show();
            $("#search-area").hide();
...
function nextToUserPage() {
    if ($(".tenant-intermediate-database-form").find(".has-error").length == 0 || $("#skip-intermediate-db").is(":checked")) {
        ...
        $("#details-next").attr("value", "Create and launch site");
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result == true) {
                hideWaitingPopup(waitingPopUpElement);
                parent.$("#add-tenant-popup").ejDialog("close");
                parent.messageBox("su-edit", "Update Site", "Site has been updated successfully", "success", function () {
                ...
                hideWaitingPopup(waitingPopUpElement);
                parent.$("#add-tenant-popup").ejDialog("close");
                parent.messageBox("su-edit", "Update Site" + " site", "Site Updation Failed", "success", function () {
                    parent.onCloseMessageBox();
                });
            }
        }
...
function tenantNameIdentiferCheck(tenantName, tenantIdentifier) {
    ...
    if (IsValidIdentifier($.trim(tenantIdentifier))) {
                ...
                success: function (data) {
                    ...
                    if (data.Result || data.ResultIdentifier) {
                        ...
                        if (data.Result && tenantName != tenantNameinDB) {
                            ...
                            $("#tenant-name-validation-error").html("This site name already exists.");
                        } else {
                            if (actionType.toLowerCase() == "edit") {
            ...
            $("#tenant-identifier").closest("div").addClass("has-error");
            $("#tenant-identifier-validation-error").css("display", "block");
            $("#tenant-identifier-validation-error").html("Please avoid special characters.");
            if (actionType.toLowerCase() != "edit") {
                $("#tenant-name, #details-next, #tenant-identifier").removeAttr("disabled");
            } else {
                $("#tenant-name, #details-next").removeAttr("disabled");
...
function nextToDatabasePage() {
    ...
    $("#header-title").html("Select Database");
    $("#header-description").html("The place where you will create and share your " + item + ".");
    ...
    if (actionType.toLowerCase() === "edit") {
        $("#details-next").attr("value", "Update");
