        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.status) {
                    ...
                    SuccessAlert("Grant Site Access", content, 7000);
                } else {
                    var message = "An error occurred while granting access to users.";
                    WarningAlert("Grant Site Access", message, 7000);
                }
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.status) {
                    ...
                    SuccessAlert("Revoke Site Access", content, 7000);
                } else {
                    var message = "An error occurred while revoking access for users.";
                    WarningAlert("Revoke Site Access", message, 7000);
                }
...
$(document).on("click", ".tenant-action", function (e) {
    ...
    if (action === "activate") {
        ...
        messageContent += "<br/><br/><div class='tenant-delete-warning'> <span>Warning:</span><div> This will delete all the resource and database linked to this site, and this action cannot be undone.</div></div>";
    }
    if (action !== "edit") {
        messageBox("su-" + headerIcon, headerText + " site", messageContent, "error", function () {
...
function updateTenantStatus(actionUrl, tenantId, action) {
    ...
    var actionName = action === "suspend" ? "Suspend" : action === "delete" ? "Delete" : "Activate";
    $.ajax({
        type: "POST",
        url: actionUrl,
        ...
        success: function (data) {
            if (data.Success) {
                if (action === "suspend") {
                    SuccessAlert(actionName + " site", "Site has been suspended successfully.", 7000);
                }
                else if (action === "delete") {
                    SuccessAlert(actionName + " site", "Site has been deleted successfully.", 7000);
                }
                else if (action === "activate") {
                    SuccessAlert(actionName + " site", "Site has been activated successfully.", 7000);
                }
                ...
                WarningAlert(actionName + " site", "Internal server error. Please try again", 7000);
            }
        },
        complete: function () {
