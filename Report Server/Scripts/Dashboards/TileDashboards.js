...
serverApp.controller('DashboardCtrl', ["$scope", "$http", "$timeout", "$window", "ajaxService", function ($scope, $http, $timeout, $window, ajaxService) {
    $scope.activeMenu = displayCategory.AllReports;
    $scope.activeMenuLabel = window.Server.App.LocalizationContent.AllReportInCategory;
    ...
    $scope.selectedFilter = displayCategory.AllReports;
    ...
    $scope.setActive = function (value, label, $event) {
        ...
        $scope.dashboardSearchQuery = '';
        $scope.selectedFilter = $scope.selectedFilter != displayCategory.DraftItems ? $scope.selectedFilter : displayCategory.AllReports;
        $scope.expandCatSec = false;
    ...
    $scope.setWidthForDashboardName = function () {
        if ($scope.activeMenu == displayCategory.AllReports) {
            if ($scope.isTablet) {
                if ($scope.isMobile) {
                    return "col-xs-12";
                }
    ...
    $scope.onSearchFocusOut = function () {
        ...
        if ($scope.isTablet && $scope.dashboardSearchQuery == "") {
    ...
    $scope.viewSchedules = function () {
        var currentDashboardItem = $(this)[0].dashboardItem;
        var url = viewSchedulesUrl + "?report=" + currentDashboardItem.Name;
        window.open(url, "_blank");
    };
    ...
    $scope.sharePermission = function () {
        sharePermission($(this)[0].categoryItem.Id);
    };

    $scope.versionHistory = function () {
        var currentDashboardItem = $(this)[0].dashboardItem;
        VersionHistory(currentDashboardItem.Id, currentDashboardItem.Name);
    };
    ...
    $scope.updateReport = function () {
        updateReport($(this)[0].dashboardItem.Id);
    };
...
function toggleFavorite(dashboardItem, $http) {
    ...
    $http({
        method: "post",
        url: favoriteItemUrl,
        data: $.param({ itemId: dashboardItem.Id, value: dashboardItem.IsFavorite })
    }).then(function (response) {
...
function historyPush() {
    ...
    switch ($scope.filterParameter.displayCategory) {
        case displayCategory.AllReports:
