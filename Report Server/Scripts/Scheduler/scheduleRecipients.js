function getAllStaticData() {
    ...
    $("#user-search-container .bs-deselect-all").after("<div class='bs-select-all-custom'><span>[[[Select All]]]</span><span class='bs-select-custom-tick glyphicon glyphicon-ok' ></span></div>");
    $("#group-search-container .bs-deselect-all").after("<div class='bs-select-all-custom'><span>[[[Select All]]]</span><span class='bs-select-custom-tick glyphicon glyphicon-ok' ></span></div>");
}
    ...
    if (($("#save-as-file").is(":checked") || $("#enable-send-mail").is(":checked"))) {
        $.ajax({
            ...
            success: function (data) {
                ...
                switch (scheduleEndType) {
                    case "never":
                        ScheduleDetail = ScheduleDetail + " [[[effective from]]] " + data.NextSchedule;
                        break;
                    case "endafter":
                        ScheduleDetail = ScheduleDetail + " [[[effective from]]] " + data.NextSchedule + " [[[and ends after]]] " + scheduleItem.RecurrenceFactor + " [[[occurrences.]]]";
                        break;
                    case "endBy":
                        ScheduleDetail = ScheduleDetail + " [[[effective from]]] " + data.NextSchedule + " [[[until]]] " + data.EndDate.toString();
                        break;
                }
