...
$(document).ready(function () {
    ...
    var dashboardViewToggleWaitingPopupTemplateId = createLoader("report-view-toogle");
    $("#report-view-toogle").ejWaitingPopup({ template: $("#" + dashboardViewToggleWaitingPopupTemplateId) });

    $("#delete-div").ejDialog({
        width: (window.innerWidth > 460) ? "450" : (window.innerWidth - 10),
        showOnInit: false,
...
$(document).on('click', function (e) {
    ...
    if ((ele.style.display == 'block' || ele.style.display == '') && (e.target.id != "sync_report_viewer_toolbar_link")) {
        ...
        if ((!(clickedOn.parents().andSelf().is('#report-view-toogle'))) && (!(clickedOn.parents().andSelf().is("#PopupContainerDelete"))) && (!(clickedOn.parents().andSelf().is("#messageBox")))) {
            ...
            if (e.target.id == "sync_report_viewer_toolicon_com" || e.target.id == "sync_report_viewer_toolbar_com") {
                $("#sync_report_viewer").css("width", $(window).width() - 410 + "px");
            }
        }
    } else {
        var viewele = document.getElementById("report-views");
        if ((e.target.id == "sync_report_viewer_toolbar_link") && (viewele.style.display == 'none')) {
...
function openViews() {
    ...
    if (e.style.display == 'block' || e.style.display == '') {
        ...
        $(".PopupClose").addClass(".show-view-panel");
    }
    else {
        $("#header-pane").addClass("no-border");
        $(".PopupClose").removeClass(".show-view-panel");
    }
...
function CloseReportView() {
    ...
    $("#report-views").toggle();
    $(".PopupClose").removeClass(".show-view-panel");
    $("#close-view .PopupClose").css("display", "none");
    $("#sync_report_viewer").css("width", $(window).width());
    $("#expand-collapse").html('<span class="su su-sidebar-collapse"></span>');
...
function filterView(args) {
    if (!viewReportClick) {
        ...
        if (paramters[0] != null && paramters[0].Name != 'undefined') {
            ...
            $("#sync_report_viewer_toolbar_link").css("display", "none");
...
function AddView() {
    var _data = "Datas";
    var link = "";
    var proxy = $('#sync_report_viewer').data('boldReportViewer');
