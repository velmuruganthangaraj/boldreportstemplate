...
$(document).ready(function () {
    ...
    $(document).on('click', '#post-comment', function () {
        ...
        if (commentText.length > 4000) {
            ...
            $(".comment-area").val("");
            $(".error-message").html("");
            var values = { itemId: itemId, commentText: commentText, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), uniqueUsersValue: userMention.uniqueUsersValue, uniqueGroupValue: userMention.uniqueGroupValue, mentionedAll: userMention.mentionedAll, parentDashboardId: parentDashboardId };
    ...
    $(document).on('click', '.reply-comment', function () {
        ...
        $("#post-reply").on('click', function () {
            ...
            var values = { itemId: itemId, commentText: commentText, parentId: parentId, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), uniqueUsersValue: userMention.uniqueUsersValue, uniqueGroupValue: userMention.uniqueGroupValue, mentionedAll: userMention.mentionedAll, parentDashboardId: parentDashboardId };
            var addComment = "";
    ...
    $(document).on('click', '.delete-comment', function () {
        ...
        var values = { itemId: itemId, commentId: commentId, itemType: itemType, dashboardItemId: dashboardItemId, url: decodeURI(currentPageUrl), parentDashboardId: parentDashboardId };
        var deletedcomment = "";
        parent.messageBox("su-delete", window.Server.App.LocalizationContent.DeleteComment, window.Server.App.LocalizationContent.DeleteComentConfirm, "error", function () {
            ...
            window.top.$("#messageBox_wrapper").ejWaitingPopup({ template: parent.$("#" + messageboxWaitingPopupTemplateIdId) });
            window.top.$("#messageBox_wrapper").ejWaitingPopup("show");
            $.ajax({
                type: "POST",
...
function GetAllComments(itemId, itemType, dashboardItemId, orderBy) {
    var skip = 0;
    var take = 0;
    var values = "&itemId=" + itemId + "&orderBy=" + orderBy + "&skip=" + skip + "&take=" + take + "&itemType=" + itemType + "&dashboardItemId=" + dashboardItemId + "&parentDashboardId=" + parentDashboardId;
    showWaitingPopup("parent-scroll-element");
    getAllCommentAjax = $.ajax({
        type: "POST",
